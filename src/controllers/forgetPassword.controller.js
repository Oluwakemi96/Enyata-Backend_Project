...
const forgotPassword = async(req, res) => {
    ...
    try {
        const existingEmail = await db.oneOrNone(forgetPassword.existingEmail, [email_address])
        if(!existingEmail){
        ...
        const resetLink = `http://localhost:8080/forgotpassword/enter_email_address/{token}`
...
const resetPassword = async (req, res) => {
    ...
    try {
        const user = await db.oneOrNone(forgetPassword.updateUser, [password, confirm_password, email_address])
